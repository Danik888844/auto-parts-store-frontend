...
@Component({
  selector: 'app-sales-by-period-report',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: './sales-by-period-report.component.html',
  styleUrl: './sales-by-period-report.component.scss',
})
export class SalesByPeriodReportComponent implements OnInit {
  ...
  private initChartOptions(): void {
    this.series = [
      { name: this.translateService.instant('SalesByPeriod'), data: [] },
    ];
    this.chart = {
    ...
    this.yaxis = {
      labels: { formatter: (val: number) => (val == null ? '' : String(val)) },
      title: { text: '' },
    };
